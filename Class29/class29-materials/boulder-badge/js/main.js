...
let bulbasaur = 16 - 5, caterpie = 7 - 1, weedle = 7 - 1, totalCandies = bulbasaur + caterpie + weedle
//Conditionals And Functions
//You have a charmander in your party. Charmander can only battle if the temperature is above 0 degrees celcius. Create one function that converts a Fahrenheit value to Celcius and another fuction that tells you wheither or not charmander can battle
function fahToCel(x){
    return 5/9 * (x-32)
}
function charCanFight(x){
    return fahToCel(x) > 0
}
console.log(charCanFight(31))
console.log(charCanFight(33))
//Loops
//You have joined an undeground pokemon leauge. In this league, trainers can use any number of pokemon. Print to the console "Pikachu I choose you" x times where x is the number of pokemon the trainer you are battling has in their party
function anyNumPokemon(x){
    for(let i = 0; i<x.length; i++){
        console.log('Pikachu I choose you')
    }
}
console.log(anyNumPokemon('bulbasaur', 'caterpie', 'mike'))